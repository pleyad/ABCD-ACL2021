...
parser.add_argument("--reference", help="Path to the file that contains the gold sentences (one per line)",
                    type=file_path)
parser.add_argument("--hypothesis", help="Path to the file that contains the sentences generated by the model (one "
                                         "per line)", type=file_path)
parser.add_argument("--modelname", help="Name of the model that is being evaluated", type=str)
parser.add_argument("--dataset", help="Name of the dataset on which the model is being evaluated", type=str)

args = parser.parse_args()


def extract_sentences(ref_path: os.PathLike, hyp_path: os.PathLike) -> Tuple[List[List[str]], List[List[str]]]:
...
def compute_average_num_tok_per_ss(split_lines: List[List[str]]) -> float:
    ...
    for line in split_lines:
        for sent in line:
            sent_count += 1
            total_tokens += len(sent.split())  # Assuming final period counts as token too

    return total_tokens / sent_count


def compute_match_num_ss(split_ref_lines: List[List[str]], split_hyp_lines: List[List[str]]) -> float:
...
def compute_bleu_score(split_ref_lines: List[List[str]], split_hyp_lines: List[List[str]]) -> float:
    """Computes the average BLEU score"""
    bleu = BLEU(effective_order=True)  # This is recommended by sacreBLEU, no idea if it was activated in original paper
    total_bleus = 0
    for ref_line, hyp_line in zip(split_ref_lines, split_hyp_lines):
        num_sents = max(len(ref_line), len(hyp_line))
        total_bleu = 0
        for hyp, ref in zip(hyp_line, ref_line):  # aligning based on indices; we might need aligner
            bleu_score = bleu.sentence_score(hyp, [ref]).score  # up to 4grams is default; we might want ONLY 4grams
...
def compute_bert_score(split_ref_lines: List[List[str]], split_hyp_lines: List[List[str]]) -> float:
    ...
    for ref_line, hyp_line in zip(split_ref_lines, split_hyp_lines):
        ...
        for hyp, ref in zip(hyp_line, ref_line):  # aligning based on indices; we might need aligner
            ...
            total_score += f1.item()  # not clear which metric is being reported in paper, might be precision or recall
...
def main():
    ...
    print("-----------------------------\n")
    print(f"The scores for the model {args.modelname} on test set {args.dataset} are:\n")
    print(f"#T/SS: {average_num_tok_per_ss_hyp} (in reference: {average_num_tok_per_ss_ref})\n")
