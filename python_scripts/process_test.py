...
def parse_arguments():
	my_parser = argparse.ArgumentParser(description="Creates a pickle for a datasplit without gold labels (Deployment, irritatingly called 'test' in the name of the script.)")
...
sents = open(Path(args.datapath) / "test.complex.txt").readlines()
orig_sents = [s.strip() for s in sents] 

deplines = open(Path(args.datapath) / "test.complex.txt.out").readlines()
...
with open(Path(args.datapath) / 'test.pkl', 'wb') as handle:
